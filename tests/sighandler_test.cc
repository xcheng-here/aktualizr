...
TEST(SigHandler, MaskForNSeconds) {
  ...
  if ((child_pid = fork()) == 0) {
    ...
    // kill the child
    struct timeval time;
    EXPECT_EQ(gettimeofday(&time, NULL), 0);
    ...
    EXPECT_EQ(gettimeofday(&time_new, NULL), 0);

    // check that the time to kill is within reasonable bounds
    uint64_t elapsed = ((time_new.tv_sec * 1000000lu) + time_new.tv_usec) - ((time.tv_sec * 1000000lu) + time.tv_usec);
